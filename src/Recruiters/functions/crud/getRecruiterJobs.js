...
export default async function getRecruiterJobs(recruiter_id, token, setJobs) {
  try {
    const response = await axios.get(`users/recruiter/${recruiter_id}/jobs/`, {
      headers: {
    ...
    if (Array.isArray(response.data)) {
      ...
      swal({
        title: 'No Jobs Found!',
        text: 'No jobs were found for this recruiter.',
        icon: 'info',
        button: 'OK',
      });
    }
