...
//       getCompanyJobs(company_id, token, setJobs).then(() => {
//         // After jobs are set, fetch details for each job
//         if (jobs.length > 0) {
//           jobs.forEach((job) => {
//             getJobDetails(job.id, token, (updatedJob) => {
//               // Update the jobs state with the detailed information
//               setJobs((prevJobs) =>
...
//       });
//     }
//   }, [token, company_id, jobs.length]); // Ensure you include the dependencies
...
//     updateJob(job_id, jobData, token,handleClose, setJobs);
//   };
...
//             </Form.Group>
// <br />
//             <Button className="flex justify-center rounded bg-success px-6 py-2 font-medium text-gray hover:bg-opacity-90" type="submit" >
//               Save
//             </Button>
//           </Form>
//         </Modal.Body>
//         <Modal.Footer>
//           <Button className="flex justify-center rounded bg-secondary px-6 py-2 font-medium text-gray hover:bg-opacity-90" onClick={handleClose}>
//             Close
//           </Button>
//         </Modal.Footer>
//       </Modal>
...
const company_id = decodedToken.user_id;
  ...
  useEffect(() => {
    if (token) {
      getCompanyJobs(company_id, token, setJobs).then((fetchedJobs) => {
  ...
  const handleShow = () => {
    ...
    setData({
      ...
      salary: job.salary || 0,
      division: Array.isArray(job.division) ? job.division : [],
      job_type: job.job_type || "",
      job_sitting: job.job_sitting || "",
      end_date: job.end_date || "",
      is_relevant: job.is_relevant || false,
      requirements: Array.isArray(job.requirements) ? job.requirements : [],
      company: job.company || company_id,
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log("Submitting Data:", data);
    updateJob(job_id, data, token, handleClose, setJobs);
  };
